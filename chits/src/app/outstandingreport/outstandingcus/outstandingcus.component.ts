...
export class OutstandingcusComponent {
  ...
  goToPage(
    ...
    this.userData.customer().subscribe((data: any) => {
      const customer = data.object?.find(
        (item: { personalDetails: { name: string }; id: any }) =>
          item.personalDetails?.name === customerName
      );
      ...
      this.router.navigate([`${pageName}`], {
        queryParams: {
          ...
          subvalue: this.cusdata.subscription,
          routvalue: this.cusdata.value,
          cid: customerId,
          pidamt,
          inst,
          pend,
  ...
  goTobackPage(pageName: string): void {
    this.router.navigate([`${pageName}`], {
      queryParams: { brnhvalue: this.brnhvalue, brnroute: this.brnroute },
    });
  }
  custmapresponse: any;
  ListOfCustdata: any;
  cusDataToApi(data: any): void {
    this.http
      .post(this.userData.outstandingcustomer, data)
      .subscribe((data) => {
  ...
  getrecieptId(data: any): void {
    this.http
      .get(this.userData.getrecipetbycusid + data)
