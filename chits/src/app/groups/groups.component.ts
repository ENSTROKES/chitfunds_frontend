...
export class GroupsComponent implements OnInit {
  ...
  getgroupFormData(data: any): void {
    ...
    this.stringformvalue =
      data.schemeId.slabname.toString() +
      '-' +
      data.schemeId.slabinst.toString();
  ...
  getgrpMappingFormData(data: any): void {
    ...
    this.http
      .post(this.userData.creategroupMapping, data)
      .subscribe((result) => {
  ...
  searchFilter(scheme_id: any, type: any) {
    ...
    if (scheme_id != undefined && scheme_id != 'all') {
      ...
      this.http
        .get(this.userData.getAllgrp + this.url_value)
        .subscribe((data) => {
          this.grpfilres = data;
    ...
    if (
      type != undefined &&
      type != 'all' &&
      (scheme_id == undefined || scheme_id == 'all')
    ) {
      //console.log("loop - 2");
